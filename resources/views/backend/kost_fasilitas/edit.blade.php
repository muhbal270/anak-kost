            ...
            <div class="col-md-6 col-12">
                <div class="card">
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            <form class="form form-vertical" action="{{ route('admin.kost_fasilitas.update', $kostFasilitas->id) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="form-body">
                                    <div class="row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="kost_id" id="kost_id" class="form-select" aria-label="Default select example">
                                                    ...
                                                    @foreach ($kosts as $item)
                                                        <option value="{{ $item->id }}" {{ $item->id == $kostFasilitas->id ? 'selected' : '' }}>
                                                            {{ $item->nama_kost }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                                ...
                                                @foreach ($fasilitas as $item)
                                                    <div class="form-check">
                                                        <input class="form-check-input" type="checkbox" name="fasilitas_id[]" value="{{ $item->id }}"
                                                            ...
                                                            {{ in_array($item->id, $kostFasilitas->fasilitas->pluck('id')->toArray()) ? 'checked' : '' }}>
